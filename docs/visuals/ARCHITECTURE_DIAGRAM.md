...
```
1. Checkout Flow:
   Agent 1 → /api/tasks/checkout → Check plan DAG
                                  → Find ready task (no blockers)
                                  → Create lease with expiry
                                  → Return task + deadline

2. Heartbeat:
   Agent 1 → /api/tasks/{id}/heartbeat → Refresh lease expiry
                                        → Keep task locked

3. Completion:
   Agent 1 → /api/tasks/{id}/complete → Mark task done
                                       → Delete lease
...
|---------|---|
| **Path Containment** | `file_store.py` uses `os.path.realpath()` to resolve symlinks and validate paths stay within `FILES_ROOT` |
| **Lease Ownership** | Only lease holder can update task state; concurrent checkout attempts are rejected |
| **Token Protection** | Admin operations (live-file writes) require `SWITCHBOARD_ADMIN_TOKEN` header |
...
See [architecture.md](./architecture.md) for detailed API sequences and database schema.
