...
async def test_checkout_skips_blocked_tasks_until_dependencies_complete() -> None:
...
async def test_checkout_respects_maintenance_mode() -> None:
    async with AsyncSessionLocal() as session:
        service = build_task_service(session)
...
async def test_checkout_assigns_task_and_persists_lease() -> None:
    async with AsyncSessionLocal() as session:
        service = build_task_service(session)
...
async def test_complete_releases_lease_and_records_notes() -> None:
    async with AsyncSessionLocal() as session:
        service = build_task_service(session)
        task = await service.create_task(
            title="done",
