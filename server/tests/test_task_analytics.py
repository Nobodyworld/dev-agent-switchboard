...
def test_task_analytics_captures_status_and_dependency_breakdown():
    async def scenario():
...
def test_task_analytics_reports_missing_dependencies():
    async def scenario():
        async with AsyncSessionLocal() as session:
            service = build_task_service(session)
            ...
            session.add(
                TaskDependency(task_id=child.id, depends_on_task_id=9999)
            )
            await session.commit()
