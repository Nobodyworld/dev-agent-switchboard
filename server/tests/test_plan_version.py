...
async def test_plan_version_increases_on_task_create():
...
async def test_plan_version_increases_on_task_update():
    async with AsyncSessionLocal() as session:
        await create_task(
            TaskIn(title="task", description="", depends_on=[]),
            session=session,
...
async def test_plan_version_increases_on_task_delete():
    async with AsyncSessionLocal() as session:
...
async def test_plan_version_increases_on_live_file_put():
    async with AsyncSessionLocal() as session:
        before = await _fetch_plan_version(session)
...
async def test_plan_version_increases_on_repeated_task_updates():
    async with AsyncSessionLocal() as session:
        await create_task(
            TaskIn(title="task", description="", depends_on=[]),
            session=session,
...
async def test_plan_metadata_includes_timestamp():
    async with AsyncSessionLocal() as session:
...
async def test_plan_returns_completion_notes():
    async with AsyncSessionLocal() as session:
        created = await create_task(
            TaskIn(title="task", description="", depends_on=[]),
            session=session,
