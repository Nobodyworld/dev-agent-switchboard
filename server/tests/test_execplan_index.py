...
async def test_execplan_index_empty_registry_defaults():
...
async def test_execplan_index_includes_plan_metadata_and_yaml():
...
async def test_execplan_index_etag_updates_on_mutation():
    async with AsyncSessionLocal() as session:
        plan = _sample_plan()
        session.add(plan)
        await session.commit()
